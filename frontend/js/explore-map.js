...
const markerIcons = {
    temple: {
        iconUrl: 'data:image/svg+xml;base64,' + btoa('<svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 36 36"><circle cx="18" cy="18" r="16" fill="#d4af37" stroke="#fff" stroke-width="3"/><text x="18" y="24" font-size="16" text-anchor="middle" fill="#fff">T</text></svg>'),
        iconSize: [36, 36], iconAnchor: [18, 36], popupAnchor: [0, -36]
    },
    cafe: {
        iconUrl: 'data:image/svg+xml;base64,' + btoa('<svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 36 36"><circle cx="18" cy="18" r="16" fill="#2196F3" stroke="#fff" stroke-width="3"/><text x="18" y="24" font-size="16" text-anchor="middle" fill="#fff">C</text></svg>'),
        iconSize: [36, 36], iconAnchor: [18, 36], popupAnchor: [0, -36]
    },
    hotel: {
        iconUrl: 'data:image/svg+xml;base64,' + btoa('<svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 36 36"><circle cx="18" cy="18" r="16" fill="#4CAF50" stroke="#fff" stroke-width="3"/><text x="18" y="24" font-size="16" text-anchor="middle" fill="#fff">H</text></svg>'),
        iconSize: [36, 36], iconAnchor: [18, 36], popupAnchor: [0, -36]
    },
    nature: {
        iconUrl: 'data:image/svg+xml;base64,' + btoa('<svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 36 36"><circle cx="18" cy="18" r="16" fill="#66BB6A" stroke="#fff" stroke-width="3"/><text x="18" y="24" font-size="16" text-anchor="middle" fill="#fff">N</text></svg>'),
        iconSize: [36, 36], iconAnchor: [18, 36], popupAnchor: [0, -36]
    }
};
...
// Fallback nav
function setupBasicCarouselNavs() {
    const configs = [
        { id: 'carousel-recommended', prev: 'recommended-prev', next: 'recommended-next' },
        { id: 'carousel-culture', prev: 'culture-prev', next: 'culture-next' },
        { id: 'carousel-cafe', prev: 'cafe-prev', next: 'cafe-next' },
        { id: 'carousel-hotel', prev: 'hotel-prev', next: 'hotel-next' },
        { id: 'carousel-nature', prev: 'nature-prev', next: 'nature-next' }
    ];
    configs.forEach(c => {
        const carousel = document.getElementById(c.id);
        const prevBtn = document.getElementById(c.prev);
        const nextBtn = document.getElementById(c.next);
        if (!carousel) return;
        if (prevBtn) prevBtn.addEventListener('click', () => carousel.scrollBy({ left: -340, behavior: 'smooth' }));
        if (nextBtn) nextBtn.addEventListener('click', () => carousel.scrollBy({ left: 340, behavior: 'smooth' }));
    });
}
...
function setupFilterTabs() {
    ...
    const sectionMap = {
        all: null, recommended: 'recommended', culture: 'culture',
        cafe: 'cafe', hotel: 'hotel', nature: 'nature', food: 'food'
    };
    tabs.forEach(tab => {
